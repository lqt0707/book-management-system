    ...
    async function query() {
        ...
        try{
            ...
            if(res.status === 200 || res.status === 201) {
                ...
                form.setFieldValue('id', bookData.id);
                form.setFieldValue('name', bookData.name);
                form.setFieldValue('author', bookData.author);
                form.setFieldValue('description', bookData.description);
                form.setFieldValue('cover', bookData.cover);
            } 
        } catch(e: any){
            message.error(e.response?.data?.message || '获取图书详情失败');
        }
    }
